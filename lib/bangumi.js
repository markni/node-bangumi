 ...
 * @param [options.debug] {boolean=false} debug mode, shows more logs
 * @example
...
Bangumi.prototype._get = function(url, params, callback) {
  ...
  const options = {
    host: this.options.api_root,
    path: url + (querystring.stringify(params) ? '?' : '') + querystring.stringify(params),
    method: 'GET',
    headers: this.options.headers
  };

  if (this.options.debug) console.log(`[${options.method}] ${options.path}`);
 ...
 * @param [params] {object}
 * @param [params.responseGroup] {("small"|"medium"|"large")} accept [small|medium|large]
 * @param [callback] {function} callback function
 * @returns {Promise}
 */
 ...
 * @param [params.app_id] {string} app id acquired from bangumi dev dashboard
 * @param [callback] {function} callback function
 * @returns {Promise}
 * @example
...
Bangumi.prototype.fetchUserCollections = function(username, subject_type, params, callback){
  //note here using our app_id due to a api bug: https://github.com/bangumi/api/issues/24
...
/**
 * A GET method that fetch a list of persons that matching query conditions
 * @param [params] {object}
 * @param params.name [{string}] the name keyword of the person
 * @param params.type [{(1|2|3)}] the type of person (1 for individual, 2 for company, 3 for group)
 * @param params.career[] [{("producer"|"mangaka"|"artist"|"seiyu"|"writer"|"illustrator"|"actor")}] the job title(s) of the person
 * @param params.sort [{("id"|"name"|"update")="id"}] sort by
 * @param params.order [{(1|-1)}] desc or asc sorting order
 * @param params.limit [number=30] results per page
 * @param params.offset [number=0] offset
 * @param [callback] {function} callback function
 * @returns {Promise}
 * @example
 * bgm.fetchPersons({name:'庵野秀明', career:["producer"]});
 */
Bangumi.prototype.fetchPersons = function (params, callback) {
  const url = '/v0/persons';
  return this.get(url, params, callback);
};

/**
 * A GET method that get details of a person object
 * @param person_id {number} id of target person
 * @param [params] {object}
 * @param [callback] {function} callback function
 * @returns {Promise}
 * @example
 * bgm.fetchPerson(1);
 */
Bangumi.prototype.fetchPerson = function (person_id, params, callback) {
  const url = '/v0/persons/' + person_id;
  return this.get(url, params, callback);
};

/**
 * A GET method that get a list of subjects given person is involved in
 * @param person_id {number} id of target person
 * @param [params] {object}
 * @param [callback] {function} callback function
 * @returns {Promise}
 * @example
 * bgm.fetchPersonSubjects(1);
 */
Bangumi.prototype.fetchPersonSubjects = function (person_id, params, callback) {
  const url = '/v0/persons/' + person_id + '/subjects';
  return this.get(url, params, callback);
};


/**
 * A GET method that fetch a list of characters that matching query conditions
 * @param [params] {object}
 * @param params.name [{string}] the name keyword of the character
 * @param params.sort [{("id"|"name"|"update")="id"}] sort by
 * @param params.order [{(1|-1)}] desc or asc sorting order
 * @param params.limit [number=30] results per page
 * @param params.offset [number=0] offset
 ...
 * @returns {Promise}
 * @example
 * bgm.fetchCharacters({name:'定春'});
 */
Bangumi.prototype.fetchCharacters = function (params, callback) {
  const url = '/v0/characters';
  return this.get(url, params, callback);
};

/**
 * A GET method that get details of a character object
 * @param character_id {number} id of target character
 * @param [params] {object}
 * @param [callback] {function} callback function
 * @returns {Promise}
 * @example
 * bgm.fetchCharacter(1);
 */
Bangumi.prototype.fetchCharacter = function (character_id, params, callback) {
  const url = '/v0/characters/' + character_id;
  return this.get(url, params, callback);
};

/**
 * A GET method that get a list of subjects given character is involved in
 * @param character_id {number} id of target character
 * @param [params] {object}
 * @param [callback] {function} callback function
 * @returns {Promise}
 * @example
 * bgm.fetchCharacterSubjects(1);
 */
Bangumi.prototype.fetchCharacterSubjects = function (character_id, params, callback) {
  const url = '/v0/characters/' + character_id + '/subjects';
  return this.get(url, params, callback);
};
